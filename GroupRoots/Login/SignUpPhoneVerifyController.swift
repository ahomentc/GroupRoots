//
//  SignUpPhoneVerifyController.swift
//  GroupRoots
//
...
class SignUpPhoneVerifyController: UIViewController, UINavigationControllerDelegate {
    ...
    @objc private func handleVerify() {
        ...
        user.link(with: credential) { (authResult, error) in
            ...
            Database.database().doesNumberExist(number: phone, completion: { (exists) in
                if !exists{
                    Database.database().updateUser(withUID: user.uid, phone: phone) { (err) in
                        Database.database().addNumberToNumbers(number: phone) { (err) in
                            Database.database().addNumberToGroupsInvited(number: phone) { (err) in
                                Database.database().removeNumberFromInvited(number: phone) { (err) in
...
extension SignUpPhoneVerifyController: UITextFieldDelegate {
